...
// Minimal GPX parser: extract <trkpt lat=".." lon=".."> and optional <ele>..</ele>
// Produces: { name, points: [{lat,lon,alt}] }

function parseGpx(gpxText) {
  const name =
    (gpxText.match(/<name>([^<]+)<\/name>/) || [])[1] ||
    (gpxText.match(/<metadata>[\s\S]*?<name>([^<]+)<\/name>[\s\S]*?<\/metadata>/) || [])[1] ||
    "route";

  const re = /<trkpt\s+[^>]*?lat="([0-9.+-]+)"\s+[^>]*?lon="([0-9.+-]+)"[^>]*>([\s\S]*?)<\/trkpt>/g;
  let m;
  const pts = [];
  while ((m = re.exec(gpxText))) {
    ...
    const lon = +m[2];
    const inside = m[3] || "";
    const eleM = inside.match(/<ele>([0-9.+-]+)<\/ele>/);
    const alt = eleM ? +eleM[1] : null;
    pts.push({ lat, lon, alt });
  }

  // fallback for route points (<rtept>)
  if (pts.length < 2) {
    const re2 = /<rtept\s+[^>]*?lat="([0-9.+-]+)"\s+[^>]*?lon="([0-9.+-]+)"[^>]*>([\s\S]*?)<\/rtept>/g;
  ...
  if (pts.length < 2) throw new Error("GPX parse failed: no track/route points");

  return { name, points: pts, start: pts[0], end: pts[pts.length - 1] };
}
...
function importGpxFromFile(gpxPath, opts = {}) {
  ...
  const canon = canonicalRouteJson({ sourceType: "gpx", pointsSimplified });
