// Patch an OpenClaw config JSON to include the installed outsideclaw skill path.
// Safe behavior:
// - Creates a .bak backup next to the config.
// - Only adds skill if not present.
// Usage:
//   node tools/patch_openclaw_config.js --config /path/to/config.json
...
if (!configPath) {
  console.error("Usage: node tools/patch_openclaw_config.js --config /path/to/config.json");
  process.exit(1);
}
